...
function huskiesLost(game) {
  if(
      (game.opponent === 'UW' && game.opponent_score < game.home_score) ||
      (game.opponent !== 'UW' && game.opponent_score > game.home_score)
    ) {
    return true;
  } else {
...
var gameLost = huskyGames2016.filter(huskiesLost);
console.log(gameLost.map(extractOpponent));
...
gameLost.forEach(function(game){
...
console.log(huskyGames2016.filter(function (game) {
  if (game.fumbles > 0) {
    return true;
  } else {
    return false;
...
function mostYardsPassing(gameOne, gameTwo) {
  if (gameOne.passing_yards === null || gameOne.passing_yards < gameTwo.passing_yards) {
    return gameTwo;
  } else {
    return gameOne;
  }
...
function makeCombinedFilter(funcOne, funcTwo) {
  function combinedFilter (game) {
    return funcOne(game) && funcTwo(game);
  }
...
function gamesWithFumble(game) {
  return game.fumbles > 0;
}
var fumbledAndLostFilter = makeCombinedFilter(huskiesLost, gamesWithFumble);
...
var fumbledLostGame = huskyGames2016.filter(fumbledAndLostFilter);
console.log(fumbledLostGame);
